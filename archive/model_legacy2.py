...
class Classifier:
    ...
    def fit(self, csv_file, csv_file_val, num_epochs, num_records, num_val_records, resume=True):
        print("start training model")
       
        device=self.device

        dataset = FlowRecordDataset(csv_file, num_records)
        balance = self.balance
        ...
        val_loader = self.load_val_dataset(csv_file_val, num_val_records)
        ...
        if os.path.isfile(filepath):
            ...
            if 'training_completedummy' in checkpoint:# do not execute temporarily, debugging 
                ...
                print('Resuming from epoch ', best_epoch)
            best_acc, acc = self.get_val_accuracy(model, val_loader)
            model.train()
        ...
        for epoch in range(resume_epoch,num_epochs):
            train_loader = utils.DataLoader(FlowRecordDataset(csv_file, num_records),batch_size=self.batch_size)
            print("epoch:", epoch)
            epoch_started = time.time()
            num_batches = len(train_loader)
            print("Total train examples (Given vs loaded)\n:  {} vs. {} // #batches in train loader: {}".format(num_records,num_batches*self.batch_size, num_batches))
            ...
            for i,batch in enumerate(train_loader):
                ...
                print("batch[{}] = {}" .format(i,batch[0].shape))
                ...
                if i%validation_interval_iters==0:
                    #print("Validating")
                    tick = time.time()
                    balanced_acc, acc = self.get_val_accuracy(model, val_loader)
                    ...
                    print ('Epoch [{}/{}], Step [{}/{}], Loss: {:.4f}, Balanced train/Val Acc: {:.2f}/{:.2f}' 
                                               .format(epoch+1, num_epochs, i+1, num_records//self.batch_size, loss.item(), train_acc, balanced_acc))
    ...
    def get_val_accuracy(self,model, loader):
        #print('\n----------Validating----------')

        torch.set_grad_enabled(False)
        model.eval()        
        y = torch.cat([y for i, (x,y) in tqdm(enumerate(loader))], dim=0)
        outputs = torch.cat([model(x.to(self.device)) for i, (x,y) in tqdm(enumerate(loader))], dim=0)
        _, pred = torch.max(outputs.data,1)
