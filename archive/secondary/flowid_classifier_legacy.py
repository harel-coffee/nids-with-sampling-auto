...
SEED = 234
fingerprint = None
use_class_weight_to_balance = False
...
def get_classifier(classifier_args,args,runs_dir=None):
    ...
    if classifier_name=='tree':
        ...
        if runs_dir is None:
            ...
            exit(1)
        (_,optim,lr,reg,batch_size,input_dim,num_class,num_iters) = args
        clf = Classifier(method='softmax',input_dim = input_dim,num_classes=num_class,lr=lr,reg=reg,num_iters=num_iters,batch_size=batch_size,runs_dir=runs_dir)
        return clf
        ...
        (_,optim,lr,reg,batch_size,input_dim,num_class,num_iters,device) = args
        clf = Classifier(method='cnn2',input_dim=input_dim,num_classes=num_class,lr=lr,reg=reg,num_iters=num_iters,batch_size=batch_size,runs_dir=runs_dir,device=device)
        return clf


def classify_fold(classifier_args,X_train,y_train,X_test,fold_index,args,runs_dir):
    classifier_name = args[0]
    ...
    tock = time.time()
    duration = tock-tick
    print("Predicted data of size ",X_test.shape," in {0:.2f} sec.".format(duration))
...
def classify(dataroot,classifier_name,file_ending):
        global K
        K=5        
...
if __name__=='__main__':
    tick = time.time()    
    classify('/home/juma/data/net_intrusion/CIC-IDS-2018/CSVs/without_sampling_l', 'forest','*Meter.csv') 
    tock = time.time()
